...
void generateNetworkMarker()
{
    ...
    for (auto &[uav_id, uav_pose] : uav_poses)
    {
        ...
        {
            // Special treatment of non-neighbour "neighbours"
            if (!uav_neighbours[uav_id].count("target_" + std::to_string(target_id)))
                addWeakEdge(uav_pose, target_pose);
            else
                addEdge(uav_pose, target_pose);
        }
    }
    edge_marker_array.markers.push_back(edge_list);

    // UAV_BS
    edge_list.ns = "uav_bs_edge";
    edge_list.points.clear();
    edge_list.colors.clear();
    for (auto &[uav_id, uav_pose] : uav_poses)
    {
        ...
        {
            // Special treatment of non-neighbour "neighbours"
            if (!uav_neighbours[uav_id].count("bs_" + std::to_string(bs_id)))
                addWeakEdge(uav_pose, bs_pose);
            else
                addEdge(uav_pose, bs_pose);
        }
    }
    edge_marker_array.markers.push_back(edge_list);

    // TARGET_BS
    edge_list.ns = "target_bs_edge";
    edge_list.points.clear();
    edge_list.colors.clear();
    for (auto &[bs_id, bs_pose] : bs_poses)
    {
    ...
    // TARGET_TARGET
    edge_list.ns = "target_target_edge";
    edge_list.points.clear();
    edge_list.colors.clear();
