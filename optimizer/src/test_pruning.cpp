        ...
        for (size_t idx = 0; idx < E.size(); ++idx)
        {
            ...
            for (auto [v1, v2] : {pair<nodeid_type, nodeid_type>{curr_edge.vertex1, curr_edge.vertex2}, pair<nodeid_type, nodeid_type>{curr_edge.vertex2, curr_edge.vertex1}})
            {
                ...
                if (!Index_to_NodePtr.count(v2))
                    Index_to_NodePtr[v2] = new SteinerTreeNode(v2);
...
{
    if (from->id == 1 && to->id == 8)
        return true;
    if (from->id == 1 && to->id == 9)
        return true;
    if (from->id == 1 && to->id == 11)
        return true;
    if (from->id == 2 && to->id == 6)
        return true;
    if (from->id == 2 && to->id == 7)
        return true;
    if (from->id == 5 && to->id == 7)
        return true;
    return false;
}
...
void Prune(SteinerTreeNode *root)
{
    ...
    if (root->id != dummy_root_id)
    {
        ...
        while (!Q.empty())
        {
            SteinerTreeNode *curr_child = Q.front();
            Q.pop();
            if (!isDeletable(curr_child))
                continue;
