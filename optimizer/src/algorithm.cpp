...
void setInputMapCallback(const sensor_msgs::PointCloud2ConstPtr &global_map)
{
    ...
    if (!scp_generator.global_pcl.empty())
        scp_generator.global_kd_tree.setInputCloud(scp_generator.global_pcl.makeShared());
}
...
message_files::PoseStampedArray solveOptimalPoseCentrally(const std::set<std::shared_ptr<SteinerTreeNode>> &tree_node_set, StarConvexOptimizer &optimizer)
{
    ...
    for (shared_ptr<SteinerTreeNode> TreeNodePtr : tree_node_set)
    {
        ...
        for (shared_ptr<SteinerTreeNode> neighbour : TreeNodePtr->neighbours)
        {
            ...
            if (neighbour_name_id[0] == 'u')
                neighbour_name_id = "solution_" + neighbour_name_id.substr(4);
    ...
    for (unsigned uav_id = 0; uav_id < available_uav_num; ++uav_id)
    {
        ...
        if (optimal_pose == Eigen::Vector3d::Zero())
            continue;
...
int main(int argc, char **argv)
{
    ...
    while (scp_generator.global_pcl.empty())
        ros::spinOnce();
    ...
    isVisible = [&](shared_ptr<SteinerTreeNode> from, shared_ptr<SteinerTreeNode> to)
    {
        ...
        if ((from->pos - to->pos).norm() > scp_generator.radius_comm * 0.95)
            return false;
        // To improve relay to cars, use the bottom centre of grids on the ground to do more conservative pruning
        Eigen::Vector3d from_pos = from->pos;
        if (from_pos(2) < grid_resolution)
            from_pos(2) = 0.0;
        Eigen::Vector3d to_pos = to->pos;
        if (to_pos(2) < grid_resolution)
            to_pos(2) = 0.0;
    ...
    while (ros::ok())
    {
        ...
        if (!topology_initialized)
        {
            ...
            while (scp_generator.target_poses.empty())
                ros::spinOnce(); // Update targets' poses (waypoints)
